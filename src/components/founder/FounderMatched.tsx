interface FounderMatchedProps {
  clock: string;
  fProfile: boolean;
  fProfileLabel: string;
  toggleFProfile: () => void;
}

export function FounderMatched({ clock, fProfile, fProfileLabel, toggleFProfile }: FounderMatchedProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', animation: 'noct-rise .4s ease both' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 22 }}>
        ...
        <div>
          <h4 style={{ margin: '0 0 2px' }}>Aino Virtanen</h4>
          <div style={{ fontSize: 13, color: 'color-mix(in srgb, #e9e9ed 60%, transparent)' }}>Partner · Northwind Ventures</div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        <button className="btn btn-secondary btn-block" onClick={toggleFProfile}>
          {fProfileLabel}
        </button>
        {fProfile && (
          <div className="card" style={{ marginTop: 12, gap: 10, padding: 16, animation: 'noct-rise .3s ease both' }}>
            <p className="card-body" style={{ opacity: 0.8, margin: 0 }}>
              Pre-seed and seed cheques of €300k–€2M in industrial and energy software across the Nordics.
            </p>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <span className="tag tag-neutral">Pre-seed / Seed</span>
              <span className="tag tag-neutral">Energy</span>
            </div>
            <div className="card-meta">Recent: Voltaro, Nordkraft Systems, Selva</div>
          </div>
        )}
      </div>
    </div>
