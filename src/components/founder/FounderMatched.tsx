...
export function FounderMatched({ clock, fProfile, fProfileLabel, toggleFProfile, meeting }: FounderMatchedProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto', animation: 'noct-rise .4s ease both' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 22 }}>
        <Avatar logoUrl={meeting.vc.photoUrl} initials={meeting.vc.initials} size={62} fontSize={20} fit="cover" />
        <div>
          <h4 style={{ margin: '0 0 2px' }}>{meeting.vc.partner}</h4>
          <div style={{ fontSize: 13, color: 'color-mix(in srgb, #e9e9ed 60%, transparent)' }}>{meeting.vc.role} · {meeting.vc.firmName}</div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 16, paddingBottom: 8 }}>
        <button className="btn btn-secondary btn-block" onClick={toggleFProfile}>{fProfileLabel}</button>
        {fProfile && <MeetingBrief brief={meeting.briefs.founder} />}
      </div>
    </div>
  );
}
